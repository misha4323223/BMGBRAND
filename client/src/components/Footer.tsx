...
export function Footer() {
  ...
  if (s.style === "minimal") {
    return (
      <footer className="bg-secondary text-secondary-foreground border-t border-secondary-foreground/10 py-6">
        <div className="max-w-8xl mx-auto px-4 sm:px-8 lg:px-12 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-x-5 gap-y-1 text-sm text-secondary-foreground/60">
            {visibleColumns.map((col) => col.links.filter((l) => l.visible).map((link, i) => <span key={i}>{renderLink(link, `link-footer-${col.title}-${i}`)}</span>))}
          </div>
          <div className="flex gap-4">{visibleSocials.map((s, i) => <a key={i} href={s.url} target="_blank" rel="noopener noreferrer" className="text-secondary-foreground/40 hover:text-primary transition-colors" aria-label={s.platform}><SocialIcon platform={s.platform} className="w-5 h-5" /></a>)}</div>
        </div>
      </footer>
    );
  }
    ...
    return (
      <footer className="bg-secondary text-secondary-foreground py-12">
        <div className="max-w-8xl mx-auto px-4 sm:px-8 lg:px-12 text-center">
          ...
          {s.description && <p className="text-secondary-foreground/50 max-w-md mx-auto mt-3 mb-8 text-sm leading-relaxed">{s.description}</p>}
          <div className="flex flex-wrap justify-center gap-8 mb-8">
            {visibleColumns.map((col, ci) => (
              <div key={ci} className="text-left">
                <p className="text-[10px] font-mono tracking-[0.2em] uppercase text-secondary-foreground/35 mb-3">{col.title}</p>
                <ul className="space-y-2 text-sm text-secondary-foreground/65">{col.links.filter((l) => l.visible).map((link, li) => <li key={li}>{renderLink(link, `link-footer-${col.title}-${li}`)}</li>)}</ul>
              </div>
            ))}
          </div>
          {visibleSocials.length > 0 && <div className="flex justify-center gap-5 mb-8">{visibleSocials.map((soc, i) => <a key={i} href={soc.url} target="_blank" rel="noopener noreferrer" className="text-secondary-foreground/40 hover:text-primary transition-colors" aria-label={soc.platform}><SocialIcon platform={soc.platform} className="w-5 h-5" /></a>)}</div>}
          <div className="border-t border-secondary-foreground/10 pt-5 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-secondary-foreground/35">
  ...
  return (
    <footer className="bg-secondary text-secondary-foreground">
      ...
      <div className="border-b border-secondary-foreground/10 px-4 sm:px-8 lg:px-12 py-4 sm:py-5">
        <div className="flex items-center justify-between gap-4">
          ...
          {s.description && (
            <p className="hidden lg:block text-xs text-secondary-foreground/40 max-w-xs leading-relaxed">{s.description}</p>
          )}
          {visibleSocials.length > 0 && (
            <div className="flex items-center gap-4 sm:gap-5 shrink-0">
              {visibleSocials.map((soc, i) => (
                <a key={i} href={soc.url} target="_blank" rel="noopener noreferrer"
                  className="text-secondary-foreground/40 hover:text-primary transition-colors duration-200 p-1"
        ...
        {s.description && (
          <p className="lg:hidden text-xs text-secondary-foreground/40 leading-relaxed mt-3">{s.description}</p>
        )}
      </div>
      ...
      <div className="px-4 sm:px-8 lg:px-12 py-7 sm:py-10">
        <div className={`grid grid-cols-2 gap-x-4 gap-y-7 sm:gap-8 ${visibleColumns.length >= 3 ? "md:grid-cols-3 lg:grid-cols-4" : "md:grid-cols-2 lg:grid-cols-3"}`}>
          {visibleColumns.map((col, ci) => (
            <div key={ci}>
              <p className="text-[9px] sm:text-[10px] font-mono tracking-[0.22em] uppercase text-secondary-foreground/35 mb-3 sm:mb-4">{col.title}</p>
              <ul className="space-y-2 sm:space-y-2.5">
                {col.links.filter((l) => l.visible).map((link, li) => (
                  <li key={li} className="text-sm text-secondary-foreground/60 leading-snug">
      ...
      {/* Bottom bar */}
      <div className="border-t border-secondary-foreground/10 px-4 sm:px-8 lg:px-12 py-4 sm:py-5">
        ...
        <div className="flex flex-col items-center gap-3 text-xs sm:hidden">
            ...
            <div>
              {s.creditUrl ? (
                <a href={s.creditUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="text-secondary-foreground/35 text-[10px] tracking-widest uppercase font-mono">Разработано</span>
          ...
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 text-secondary-foreground/35">
          ...
          <p className="text-secondary-foreground/25">&copy; {new Date().getFullYear()} {s.copyrightText}</p>
        </div>

        {/* Desktop: row layout */}
        <div className="hidden sm:flex items-center justify-between gap-4 text-xs">
          <p className="text-secondary-foreground/35">&copy; {new Date().getFullYear()} {s.copyrightText}. Все права защищены.</p>
          <div className="flex flex-wrap justify-center gap-x-5 gap-y-1 text-secondary-foreground/35">
