...
## 6. Автоматический деплой через GitHub Actions

Если на вашем компьютере не работает Docker, вы можете настроить автоматический деплой через GitHub. Каждый раз, когда вы будете загружать код в GitHub, он сам будет обновлять ваш сайт в Яндексе.

### Настройка секретов в GitHub:
Перейдите в ваш репозиторий -> **Settings** -> **Secrets and variables** -> **Actions** -> **New repository secret** и добавьте следующие ключи:

1.  `YC_SA_JSON_KEY`: Содержимое вашего JSON-ключа сервисного аккаунта Яндекса.
2.  `YC_REGISTRY_ID`: ID вашего Container Registry (например, `crp12345...`).
3.  `YC_SA_ID`: ID вашего сервисного аккаунта.
4.  `YC_TOKEN`: Ваш OAuth-токен или IAM-токен Яндекса.

После этого любой "Push" в ветку `main` запустит автоматическое обновление сайта.
