...
## СТОП: запрет на выдуманные товары
КРИТИЧЕСКИ ВАЖНО: рекомендуй ТОЛЬКО товары из раздела «Каталог» или «Похожие товары» этого промта. Если подходящих товаров в данных нет — скажи «загляни в каталог, там найдёшь что-то подходящее» и НЕ НАЗЫВАЙ ни одного конкретного товара которого нет в переданных данных. Нарушение этого правила — серьёзная ошибка.`,
...
- Бесплатная доставка: в пункт выдачи (ПВЗ) и Ozon — при заказе от 5 000 ₽, курьером до двери — от 15 000 ₽
...
- **Ozon Доставка**: доставка через логистику Ozon (до ПВЗ)
...
**Доставка:** оплачивается оптовиком. Доступные транспортные компании: СДЭК, ПЭК, Почта России, Байкал Сервис. По другим ТК — уточняйте у менеджера.
...
**Уровни:**
- **Уровень 1** — после первой покупки → скидка **5%**
- **Уровень 2** — от 10 000 ₽ суммарных покупок → скидка **7%**
- **Уровень 3** — от 20 000 ₽ суммарных покупок → скидка **10%**
- **Уровень 4** — от 30 000 ₽ суммарных покупок → скидка **15%**
- **Уровень 5** — от 50 000 ₽ суммарных покупок → скидка **20%**
...
Если подходящей вакансии нет — можно отправить резюме напрямую: hr@booomerangs.ru. Мы рассматриваем все заявки.`,
  ...
  ai_block_brand: `## О бренде BOOOMERANGS (BMGBRAND)

**BOOOMERANGS** — российский бренд одежды и аксессуаров из Тулы. Да-да, это там, где оружейные заводы и самовары.

### История
- **2006** — открыт первый магазин StreetWear
- **2019** — запущено собственное производство полного цикла в Узловском районе Тульской области
- **2020** — в пандемию вышли первые мемные носки: принты соды, соли, сахара — то, чего тогда не было на полках. Покупатели оценили, производство начало расти
- Бренд развивается несмотря на появление конкурентов-подражателей

### Основатель
Бренд основал **Евгений Соболев**. Его принцип: создаёт только то, что нравится ему самому. Сначала вдохновение — потом эскиз, тщательный подбор тканей, многократное тестирование (стирка + носка), и только потом массовое производство.

### Философия
> «Делаем вещи, которые носим сами»

Одежда BOOOMERANGS — преимущественно **оверсайз**, **унисекс**, для тех, кто ценит комфорт и самовыражение. Дизайн вдохновлён ностальгией, ретро и советской эстетикой: сгущёнка, панельки, гжель, мемы и поп-культура.

### Производство
- Полный цикл в России: пошив, печать, упаковка — всё своё
- Ассортимент: футболки, худи, свитшоты, куртки, брюки, джоггеры, носки (200+ моделей), шопперы, кепки, шапки, сумки, аксессуары
- Торговая марка №820056 — BOOOMERANGS (зарегистрирована официально)

### Мерч и коллаборации
- Производят официальный мерч для артистов, брендов и мероприятий
- Делали мерч для фестиваля **«Дикая Мята»**
- Работают с музыкантами, блогерами, инфлюенсерами — для них доступна персональная страница на сайте: booomerangs.ru/@slug

### Платформа для медийных партнёров
Собственная платформа на сайте позволяет артистам и блогерам создавать персональные страницы: hero с видео/фото, галерея, описание, витрина мерча, именной промокод для подписчиков.

### Где найти
- Сайт: booomerangs.ru
- ВКонтакте: vk.ru/bmgbrand
- Instagram: instagram.com/bmgbrand
- Продукция представлена в 200+ мультибрендовых магазинах StreetWear по всей России и на маркетплейсах

### Признание
Участник VI федерального форума «Российская креативная неделя». Признан частью экосистемы креативных индустрий Тульской области.`,
...
const AI_TOPIC_MAP: Array<{ key: AiKnowledgeKey; kw: string[] }> = [
  ...
  { key: 'ai_block_brand',       kw: ['о бренде', 'о компании', 'о вас', 'кто вы', 'расскажи о себе', 'расскажите о', 'откуда вы', 'откуда бренд', 'история бренда', 'история компании', 'как появились', 'когда основан', 'основател', 'философи', 'производств', 'где делаете', 'где шьёте', 'где шьете', 'тула', 'тульск', 'bmgbrand', 'bmg brand', 'бумеранги', 'бумеранг', 'booomerangs', 'boomerangs', 'фестиваль', 'дикая мята', 'российская одежда', 'российский бренд', 'отечественный бренд', 'чем занимаетесь', 'что за бренд'] },
];
  ...
  app.post("/api/ai/chat", authMiddleware, async (req: AuthRequest, res) => {
    try {
      ...
      if (sizeProductId) {
        ...
        if (targetProduct) {
          ...
          if (isSock) {
            ...
            sizeAdvisorContext = `\n\n## Подбор размера — активен\nТовар: "${productName}"\n\nЭТО КОСТЮМ — подбирай ДВА размера отдельно: верх и низ. Если совпадают — скажи один (например, M), если нет — укажи оба (например, верх M / низ L).\n\nПРАВИЛА:\n1. Если параметры тела уже указаны — сразу называй размеры верха и низа.\n2. Если нет — попроси обхват груди, талии, бёдер и рост.\n3. Замеры в таблице — это ИЗДЕЛИЕ (уже с припуском). Верх: найди строку где грудь изделия БЛИЖАЙШАЯ к груди тела. Низ: найди строку где талия изделия БЛИЖАЙШАЯ к талии тела.\n⚠️ ЗАПРЕЩЕНО добавлять сантиметры к параметрам покупателя — таблица уже содержит готовые размеры изделия.\n\n${tableStr}`;
          } else if (measurements.length > 0) {
            const rows = measurements.map(buildMRowStr).join("\n");
            sizeAdvisorContext = `\n\n## Подбор размера — активен\nТовар: "${productName}"\n\nПРАВИЛА (строго соблюдай):\n1. Если покупатель уже указал параметры тела — НЕ задавай вопросов, сразу называй размер.\n2. Если параметры не указаны — попроси только обхват груди и рост.\n3. НЕ перенаправляй к менеджеру, если таблица доступна.\n\nКАК СЧИТАТЬ (читай внимательно):\n⚠️ ЗАПРЕЩЕНО: никогда не добавляй сантиметры к параметрам покупателя. Не пиши "грудь + 12", "грудь + 10–15" или любую другую прибавку.\nТаблица содержит замеры ГОТОВОГО ИЗДЕЛИЯ — свобода кроя уже заложена в эти цифры.\nПРАВИЛО: покупатель назвал грудь X → найди строку, где грудь изделия БЛИЖАЙШАЯ к X → это нужный размер.\nПример: покупатель грудь 96 см, таблица: XS=96, S=100, M=104 → рекомендуй XS (ближайший к 96).\n- Всегда называй КОНКРЕТНЫЙ размер и объясни в 1 предложении почему.\n\n### Таблица замеров изделия:\n${rows}`;
          } else {
      ...
      if (pageContext?.pageType === "product" && pageContext.product) {
        ...
        const sockSizeNote = isPageSock
          ? `\n\n⚠️ НОСКИ — ПРАВИЛО РАЗМЕРОВ: размер носков (34-39, 40-45 и т.п.) — это диапазон размера ОБУВИ, НЕ рост и НЕ обхват груди. ЗАПРЕЩЕНО упоминать рост, обхват груди или параметры тела при выборе размера носков. Если спрашивают про размер — просто скажи какой размер обуви подходит под каждый диапазон (34-39 = обувь 34-39, 40-45 = обувь 40-45). Больше ничего спрашивать не нужно.`
          : "";
        pageContextStr = `\n\n## Текущий товар (пользователь смотрит эту карточку прямо сейчас)\n- Название: ${p.name}\n- Цена: ${priceStr}\n- Цвет: ${p.color || "не указан"}\n- Состав: ${p.composition || "не указан"}\n- Описание: ${(p.description || "").slice(0, 400)}\n- Наличие по размерам: ${stockStr}\n- Категория: ${category}${pageSizeTableStr}\n\nЕсли пользователь спрашивает про этот товар (состав, размеры, цвет, наличие) — отвечай на основе этих данных.${sockSizeNote}${triggerNote}${lowStockNote}${crossSellHint}${preorderNote}`;
    ...
    try {
      ...
      if (isSockProduct) {
        ...
        sizingRule =
          ...
          `- АЛГОРИТМ ДЛЯ ВЕРХА: возьми обхват груди тела покупателя. Вычти его из каждого значения в колонке "Грудь" таблицы верха. Выбери строку с НАИМЕНЬШЕЙ разницей — ЭТО И ЕСТЬ нужный размер верха. Не пересматривай результат после вычислений.\n` +
          `  Пример: грудь тела 96 см, таблица: S=132, M=140, L=148 → |132-96|=36, |140-96|=44, |148-96|=52 → наименьшая 36 → размер S. Вывод: S.\n` +
          `- АЛГОРИТМ ДЛЯ НИЗА: возьми обхват бёдер тела покупателя. Вычти его из каждого значения в колонке "Бёдра" таблицы низа. Выбери строку с НАИМЕНЬШЕЙ разницей — ЭТО И ЕСТЬ нужный размер низа. Не пересматривай результат после вычислений.\n` +
          `  Пример: бёдра тела 98 см, таблица: XS=107, S=110, M=112 → |107-98|=9, |110-98|=12, |112-98|=14 → наименьшая 9 → размер XS (если есть в наличии).\n` +
          `- Если размер есть в наличии — рекомендуй его. Если нет в наличии — перейди к следующему с наименьшей разницей.\n` +
          `- Если оба размера совпадают — назови один (например: S); если разные — укажи оба (например: верх S / низ M).\n` +
          `⚠️ ЗАПРЕЩЕНО прибавлять сантиметры к параметрам тела — таблица уже содержит замеры готового изделия со всеми припусками.\n`;
        ...
        if (!isNaN(garmentChest) && refBody) {
          ...
          if (addOn >= 20) {
            ...
            sizingRule =
              `- СВОБОДНЫЙ КРОй: найди строку, где грудь изделия БЛИЖАЙШАЯ к обхвату груди покупателя. ⚠️ Не добавляй ничего к параметрам тела — таблица уже содержит замеры готового изделия.\n`;
          } else {
            sizingRule =
      ...
      if (hasMeasurementSections) {
        ...
        const colLabels: Record<string, string> = {
          ...
          sideLength: "Дл. по боку", bottomWidth: "Шир. низа",
        };
