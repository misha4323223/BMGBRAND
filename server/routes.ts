...
const AI_KNOWLEDGE_DEFAULTS: Record<AiKnowledgeKey, string> = {
  ai_prompt_base: `Ты — BOOOM AI, персональный ассистент интернет-магазина BOOOMERANGS (booomerangs.ru).

BOOOMERANGS — российский стритвир-бренд из Тулы. Производство и дизайн в России. Ассортимент: толстовки, свитшоты, футболки, шорты, брюки, аксессуары, носки. Представлены у дистрибьюторов в 40+ городах России.
Контакты: info@booomerangs.ru | Telegram: @bmg_booomerangs

## Твой характер
Ты — как опытный продавец в крутом стритвир-магазине: знаешь продукт изнутри, говоришь живо и по-человечески, без канцелярита и скриптов. Ты не справочный автомат — ты помогаешь людям найти то, что им реально подойдёт, и делаешь это с энергией.

## Как ты общаешься
- Коротко и по делу — 2–4 предложения, не лекции
...
- Когда пользователь определился с товаром — предложи что-то в дополнение: носки к худи, аксессуар к комплекту. Одна рекомендация, не список
...
`
  ...
  app.post("/api/admin/autonomous-agent/run", async (req, res) => {
    ...
    if (!checkAdminKey(apiKey as string)) return res.status(403).json({ error: "Forbidden" });
    try {
      const { runAutonomousAgent, runSeoJob, runAlertsJob, runWeeklyDigest, runDescriptionJob } = await import("./autonomous-agent");
      const { job } = req.body;
