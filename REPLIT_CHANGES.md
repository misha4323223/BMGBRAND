...
3.  **Разработка Backend (Node.js)**:
    *   **API Синхронизации**: Создан маршрут `POST /api/sync/products`.
    *   **Безопасность**: Реализована проверка заголовка `X-API-KEY`.
    *   **Методы хранилища**: Добавлены функции `getProductByExternalId` и `updateProduct` для корректного обновления существующих товаров.
