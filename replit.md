...
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер запустился в Local Dev режиме (YDB отключен без `YDB_SA_KEY`), загрузил 1 продукт из DevStorage. Для подключения к боевой базе добавьте секрет `YDB_SA_KEY`.
