...
## Аудит безопасности и исправления (06.08.2026, 15:28)

Проведён полный аудит безопасности: автосканеры (зависимости, SAST, HoundDog) + ручной анализ кода. Ничего не изменялось до получения команды от владельца.

### Исправлено

#### 1. Hardcoded fallback-секреты (`server/routes.ts`, `server/vk.ts`)
**Было:** 4 эндпоинта `/api/sync/*` использовали `process.env.SYNC_API_KEY || "bmg-secret-key-123"`. Если переменная окружения не задана — публичный пароль из GitHub становился реальным ключом.
**VK:** `process.env.VK_ACTION_SECRET || "vk-action-secret-fallback"` — аналогично.
**Стало:**
- `/api/sync/products`, `/api/sync/orders`, `/api/sync/orders/:id`, `/api/sync/inventory` — если `SYNC_API_KEY` не задан, возвращают `503 Service misconfigured` вместо работы с fallback-ключом.
- `server/vk.ts` — fallback убран, в `verifyActionLink()` добавлена явная проверка: пустой секрет → `return false` с логом ошибки.

#### 2. Path traversal в 1C-обмене (`server/routes.ts`)
**Было:** `filename` из URL-параметра (`?filename=../../server/config.ts`) передавался напрямую в `path.resolve()` без проверки в GET-обработчике (строка ~4824). POST-обработчик (строка ~8256) — та же проблема.
**Стало:** оба места — `path.basename(filename as string)` срезает все директории. `../../etc/passwd` → `passwd`, которого в `1c_uploads` нет.

#### 3. API-ключ в URL (`server/routes.ts`)
**Было:** 50 мест принимали ключ через `req.headers["x-api-key"] || req.query.key`. Передача через `?key=` писала admin-ключ в логи Yandex Cloud Gateway, историю браузера и access-логи.
**Стало:** все 50 мест — только `req.headers["x-api-key"]`. URL-параметр `?key=` больше не принимается нигде.

### Проверено, не требует исправления

- **1C toggle (вкл/выкл)** — работает корректно. Оба обработчика проверяют `is1CSyncEnabled`. Поведение после рестарта (сброс в `true`) — осознанное архитектурное решение владельца.
- **localhost в CORS** — оставлено намеренно для локальной разработки в Cursor. В Yandex Cloud Gateway не усиливает риск; реального вектора атаки для покупателей нет.
- **500 МБ лимит 1C** — на уровне Yandex Cloud API Gateway стоит `maxRequestBodySize: 20971520` (20 МБ), DoS через тяжёлые запросы фактически закрыт инфраструктурой.

### Выявлено, отложено (не критично сейчас)

| Находка | Причина отложить |
|---------|-----------------|
| Токен Ozon Pay в stderr (`server/ozon-pay.ts`) | Ozon Pay не используется в продакшене |
| `dangerouslySetInnerHTML` в блоге без санитизации | Только при взломе admin-аккаунта; блог редактируется редко |
| CSP отключён (`server/index.ts`) | Включение требует тщательного тестирования всего UI |
| 5 npm-пакетов с HIGH уязвимостями | Требует отдельного анализа совместимости |

### Архитектурный контекст (Yandex Cloud)
Продакшен работает в Serverless Container за API Gateway. Весь трафик идёт через Gateway — прямого доступа к контейнеру извне нет. Это закрывает часть векторов на уровне инфраструктуры.

---

## SEO-оптимизация страницы /merch-na-zakaz (20.07.2026)

Выполнено по рекомендациям SEO-оптимизатора:

1. **Bot SSR** (`server/bot-ssr.ts`) — добавлена функция `renderMerchOrder()`. Яндекс и Google теперь получают полный HTML вместо пустого `<div id="root">`. Включает H1, H2 ("Футболки с логотипом на заказ", "Худи и толстовки с принтом", "Носки с символикой компании"), тексты про шелкографию/термотрансфер/вышивку, B2B-условия, геопривязку (Москва, СПб), "Наши преимущества", "Часто заказывают", FAQ, JSON-LD (LocalBusiness + Service + HowTo + FAQPage + BreadcrumbList). Подключён в middleware после `/faq`.
2. **Sitemap** (`server/routes.ts`) — `/merch-na-zakaz`: `changefreq: monthly → weekly`, `priority: 0.8 → 0.9`. `lastmod` уже был динамическим (today).
...
### Повторный импорт (06.08.2026, восьмой раз)
...
### Повторный импорт (05.08.2026, пятый раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 846 товаров — приложение полностью рабочее.

### Повторный импорт (05.08.2026, четвёртый раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 846 товаров — приложение полностью рабочее.

### Повторный импорт (05.08.2026, третий раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 846 товаров — приложение полностью рабочее.

### Повторный импорт (05.08.2026, второй раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 846 товаров — приложение полностью рабочее.

### Повторный импорт (05.08.2026)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 846 товаров — приложение полностью рабочее.

### Повторный импорт (30.07.2026, второй раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 832 товара — приложение полностью рабочее.

### Повторный импорт (30.07.2026)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 832 товара — приложение полностью рабочее.

### Повторный импорт (29.07.2026)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 842 товара — приложение полностью рабочее.

### Повторный импорт (26.07.2026)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты добавлены пользователем после старта. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 840 товаров — приложение полностью рабочее.

### Повторный импорт (23.07.2026, третий раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 840 товаров — приложение полностью рабочее.

### Повторный импорт (23.07.2026, второй раз)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 840 товаров — приложение полностью рабочее.

### Повторный импорт (23.07.2026)
Тот же сценарий: после импорта `node_modules` был пуст, workflow завис на подтверждении установки `tsx`. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 840 товаров — приложение полностью рабочее.

### Повторный импорт (20.07.2026, четвёртый раз)
...
### Повторный импорт (20.07.2026, третий раз)
Тот же сценарий: после импорта `node_modules` был пуст. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 827 товаров — приложение полностью рабочее.

### Повторный импорт (20.07.2026, второй раз)
Тот же сценарий: после импорта `node_modules` был пуст. Все секреты (YDB, платежи, CDEK, Telegram, VK, Yandex Storage, DaData, JWT, SMTP и др.) были добавлены пользователем заранее. Исправлено: `npm install --include=dev` → перезапуск workflow. Сервер подключился к боевой YDB, загрузил 827 товаров — приложение полностью рабочее.
