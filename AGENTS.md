...
- **Правила доставки (уточнены владельцем 15.09.2026)**: пересылаем ТОЛЬКО ответы
  ...
  приходят ещё и заявки и они не должны попадать в чат клиента. Если ответ адресован не нашему
  уведомлению (`getSessionIdByVkMessageId` не нашёл сессию) — тоже пропускаем, фолбэка «в последний диалог»
  больше НЕТ. Свои исходящие (`from_id === -group_id`) и чужие peer отбрасываются,
  дубли Callback/Long Poll гасятся по id входящего сообщения.
